...
#Abundant microbes at the reproductive stage

data <- read.table("merged_abundance_table_species_study1.txt",sep="\t")
data <- as.matrix(data)
data2 <- log2(data + 1)
Heatmap(data2,
            col=colorRamp2(myBreaks, myCol),
...
data3 <- read.table("merged_abundance_table_species_study2.txt", sep="\t", header=T, row.names=1)
data3 <- as.matrix(data3)
data3 <- log2(data3 + 1)
Heatmap(data3, col=colorRamp2(myBreaks, myCol),
            name="Abundance Z Score", heatmap_legend_param=list(
              color_bar="continuous",
              legend_direction="vertical",
              legend_width=unit(5,"cm"),
